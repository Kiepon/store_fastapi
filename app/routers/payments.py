...
# Список сетей/адресов ЮКасса (Яндекс.Касса) для проверки источника вебхука
...
def is_ip_allowed(ip: str | None) -> bool:
    if ip is None: 
        return False
    try:
        address = ipaddress.ip_address(ip) # Преобразование в объект IP-адреса
    except ValueError:
        return False
    
    for mask in YANDEX_IP_LIST: # Перебор элементов списка
        if "/" in mask: # Интерпретируется как маска подсети и проверяется вхождение адреса в эту сеть.
            if address in ipaddress.ip_network(mask, strict=False): 
                return True
            else: # Если нет слеша, то проверяется полное совпадение IP адреса.
                if address == ipaddress.ip_address(mask):
                    return True
    return False


# Определяет корректный IP-адрес клиента в продакшене.
