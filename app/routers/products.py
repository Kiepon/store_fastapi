...
async def get_all_products(
    ...
    category_id: int | None = Query(
        None, ge=0, description="ID категории для фильтрации",),
    search: str | None = Query(None, min_length=1, description="Поиск по названию/описанию товара"),
...
async def get_products_by_category(category_id: int, db: AsyncSession = Depends(get_async_db)):
    ...
    """
    stmt_category = await db.scalars(select(CategoryModel).where(CategoryModel.id == category_id, 
                                                CategoryModel.is_active == True))
